...
export function DashboardNav({ user, teams }: DashboardNavProps) {
      ...
      >
        ...
        <div className="fixed inset-0 flex">
          <div className="relative mr-16 flex w-full max-w-xs flex-1">
            ...
            <SidebarContent user={user} teams={teams} pathname={pathname} />
          </div>
        </div>
      </div>
              ...
              {teams.map((team) => (
                <li key={team.id}>
                  <Link
                    ...
                    className={cn(
                      pathname.startsWith(`/dashboard/teams/${team.slug}`)
                        ? 'bg-gray-50 text-primary-600'
                        : 'text-gray-700 hover:bg-gray-50 hover:text-primary-600',
                      'group flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6',
                    )}
                  >
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border bg-white text-[0.625rem] font-medium">
