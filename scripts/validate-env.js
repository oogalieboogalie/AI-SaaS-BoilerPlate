...
function validateEnvFile() {
  ...
  if (!fs.existsSync(envPath)) {
    ...
    console.log('💡 Copy .env.example to .env.local and configure your environment');
    return false;
  }
...
function validateStripeConfiguration() {
  ...
  const isTestMode = stripePublic.startsWith('pk_test_') && stripeSecret.startsWith('sk_test_');
  const isLiveMode = stripePublic.startsWith('pk_live_') && stripeSecret.startsWith('sk_live_');
...
function main() {
  ...
  console.log('🎉 Environment validation passed! Ready to start the application.');
}
